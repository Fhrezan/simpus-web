...
// --- Kode terintegrasi akan disisipkan di sini ---
...
db.collection("patients").add(newPatient).then(docRef => {
  feedback.textContent = `Pasien berhasil didaftarkan dengan ID: ${docRef.id}`;
  feedback.style.color = "#27ae60";
  form.reset();
  loadPatientsFromFirebase().then(renderPatients);
});
...
db.collection("medicalRecords").add(newRecord).then(() => {
  feedback.textContent = "Rekam medis berhasil disimpan.";
  feedback.style.color = "#27ae60";
  form.reset();
  form.recordDate.value = new Date().toISOString().substring(0,10);
  loadMedicalRecordsFromFirebase().then(() => renderMedicalRecord(patientId));
});
          form.recordDate.value = new Date().toISOString().substring(0,10);
          // Refresh the list
          setTimeout(() => {
...
async function loadMedicalRecordsFromFirebase() {
  const snapshot = await db.collection("medicalRecords").get();
  for (const doc of snapshot.docs) {
    const data = doc.data();
    if (!medicalRecords[data.patientId]) medicalRecords[data.patientId] = [];
    medicalRecords[data.patientId].push(data);
  }
}
  ...
  await loadMedicalRecordsFromFirebase();
